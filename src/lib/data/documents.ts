...
export async function getDocuments(
  ...
  let query = supabase
    ...
    .select(`
      *,
      uploader:family_members!documents_uploaded_by_fkey(id, name)
    `)
 ...
 */
export async function getExpiringDocuments(familyId: string, days = 90) {
  const supabase = await createClient()
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      uploader:family_members(id, name)
    `)
    .eq('family_id', familyId)
