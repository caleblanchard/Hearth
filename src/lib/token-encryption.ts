 ...
 * Uses TOKEN_ENCRYPTION_SECRET as the encryption key with PBKDF2 key derivation.
 *
...
/**
 * Derive encryption key from TOKEN_ENCRYPTION_SECRET using PBKDF2
 */
function getEncryptionKey(): Buffer {
  const secret = process.env.TOKEN_ENCRYPTION_SECRET;

  if (!secret) {
    throw new Error('TOKEN_ENCRYPTION_SECRET must be set for token encryption');
  }
