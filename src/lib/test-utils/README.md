...
- **`src/lib/test-utils/__tests__/supabase-mocks.test.ts`** - Example tests demonstrating usage
...
## Migrating from Legacy DB Mocks

### Before (DB Mock)
```typescript
import { dbMock } from '@/lib/test-utils/db-mock'

dbMock.family.findMany.mockResolvedValue(mockFamilies)
```
...
npm test -- src/lib/test-utils/__tests__/supabase-mocks.test.ts

# Run with coverage
npm test -- src/lib/test-utils/__tests__/supabase-mocks.test.ts --coverage
```

## Example: Full Integration Test

See `src/lib/test-utils/__tests__/supabase-mocks.test.ts` for comprehensive examples of all patterns.
