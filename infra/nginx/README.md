...
### Change Domain
1. Update `infra/nginx/conf.d/hearth.conf`:
   - Replace `hearth.blanchardsd.com` with your domain
2. Update `infra/docker/docker-compose.nginx.yml`:
   - Replace `hearth.blanchardsd.com` in Certbot commands
3. Update email in `infra/docker/docker-compose.nginx.yml`:
...
If your Hearth app container has a different name, update `infra/nginx/conf.d/hearth.conf`:
```nginx
