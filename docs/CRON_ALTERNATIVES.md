  ...
  sync-calendars:
    ...
    steps:
      - name: Sync External Calendars
        run: |
          curl -X POST https://your-app.vercel.src/app/api/cron/sync-external-calendars \
...
3. Set URL: `https://your-app.vercel.src/app/api/cron/sync-external-calendars`
...
# Test in production
curl -X POST https://your-app.vercel.src/app/api/cron/generate-chore-instances \
  -H "Authorization: Bearer $CRON_SECRET"
```
