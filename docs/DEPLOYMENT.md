...
# Download the infra/docker/docker-compose.prod.yml file
curl -O https://raw.githubusercontent.com/caleblanchard/Hearth/master/infra/docker/docker-compose.prod.yml
```
...
TOKEN_ENCRYPTION_SECRET=your_generated_secret_here

# Set your production URL
NEXT_PUBLIC_APP_URL=http://your-server-ip:3000
# Or if using a domain:
# NEXT_PUBLIC_APP_URL=https://hearth.yourdomain.com
...
- Generate `TOKEN_ENCRYPTION_SECRET` using: `openssl rand -base64 32`
...
- `http://your-server-ip:3000` (or your configured NEXT_PUBLIC_APP_URL)
...
Then remove or comment out the `hearth-minio` service from `infra/docker/docker-compose.prod.yml`.
...
docker-compose -f infra/docker/docker-compose.prod.yml logs hearth-app
...
```bash
# Create a backup
docker-compose -f infra/docker/docker-compose.prod.yml exec hearth-db pg_dump -U hearth_user hearth_db > backup_$(date +%Y%m%d_%H%M%S).sql

# Or use this script (create as backup.sh)
#!/bin/bash
BACKUP_DIR="/opt/hearth/backups"
mkdir -p $BACKUP_DIR
docker-compose -f infra/docker/docker-compose.prod.yml exec -T hearth-db pg_dump -U hearth_user hearth_db | gzip > $BACKUP_DIR/backup_$(date +%Y%m%d_%H%M%S).sql.gz
...
docker-compose -f infra/docker/docker-compose.prod.yml up -d hearth-db

# Wait for database to be ready
sleep 10

# Restore the backup
docker-compose -f infra/docker/docker-compose.prod.yml exec -T hearth-db psql -U hearth_user hearth_db < your_backup.sql
...
2. Test database connection:
   ```bash
   docker-compose -f infra/docker/docker-compose.prod.yml exec hearth-db psql -U hearth_user hearth_db -c "SELECT version();"
   ```
...
1. Verify admin user was created:
   ```bash
   docker-compose -f infra/docker/docker-compose.prod.yml exec hearth-db psql -U hearth_user hearth_db -c "SELECT email, name, role FROM family_members WHERE role = 'PARENT';"
   ```
...
3. Check database size:
   ```bash
   docker-compose -f infra/docker/docker-compose.prod.yml exec hearth-db psql -U hearth_user hearth_db -c "SELECT pg_size_pretty(pg_database_size('hearth_db'));"
   ```
...
1. Can't access MinIO Console:
   ...
   docker-compose -f infra/docker/docker-compose.prod.yml ps hearth-minio

   # Check MinIO logs
   docker-compose -f infra/docker/docker-compose.prod.yml logs hearth-minio
...
2. Files not uploading to MinIO:
   ...
   docker-compose -f infra/docker/docker-compose.prod.yml exec hearth-app env | grep S3

   # Test MinIO connectivity from app container
   docker-compose -f infra/docker/docker-compose.prod.yml exec hearth-app curl -I http://hearth-minio:9000/minio/health/live
   ```
...
3. MinIO password not working:
   ...
   - After changing the password, restart the containers:
     ```bash
     docker-compose -f infra/docker/docker-compose.prod.yml restart hearth-minio
     ```
