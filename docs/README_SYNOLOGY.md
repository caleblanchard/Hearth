...
1. **Deploy Hearth Stack**
   - Use `infra/docker/docker-compose.synology.yml` in Portainer
   - Or use `infra/docker/docker-compose.prod.yml` (same configuration)
...
```env
NEXT_PUBLIC_APP_URL=https://hearth.blanchardsd.com
AUTH_TRUST_HOST=true
TOKEN_ENCRYPTION_SECRET=your-secret-min-32-chars
```
...
- **Mixed content:** Ensure `NEXT_PUBLIC_APP_URL` uses `https://`
