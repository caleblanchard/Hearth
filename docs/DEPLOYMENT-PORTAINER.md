...
services:
  ...
  hearth-app:
    ...
    ports:
      - "${APP_PORT:-3000}:3000"
    volumes:
      - hearth-uploads:/src/app/uploads  # Fallback for local storage if needed
    depends_on:
      hearth-db:
...
| Viewing Logs | `docker-compose -f infra/docker/docker-compose.prod.yml logs` | Click → Logs button |
| Updating Variables | Edit `.env` file | Edit in web interface |
| Restarting Services | `docker-compose -f infra/docker/docker-compose.prod.yml restart` | Click → Restart button |
