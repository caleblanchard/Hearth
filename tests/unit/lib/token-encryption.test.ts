...
describe('Token Encryption', () => {
  ...
  beforeAll(() => {
    // Set a test secret
    process.env.TOKEN_ENCRYPTION_SECRET = 'test-secret-for-encryption-minimum-32-characters-long';
  });
  ...
  describe('encryptToken', () => {
    ...
    it('should throw error if TOKEN_ENCRYPTION_SECRET is not set', () => {
      delete process.env.TOKEN_ENCRYPTION_SECRET;

      expect(() => encryptToken('test-token')).toThrow('TOKEN_ENCRYPTION_SECRET must be set');
  ...
  describe('decryptToken', () => {
    ...
    it('should throw error if TOKEN_ENCRYPTION_SECRET is not set', () => {
      ...
      expect(() => decryptToken(encrypted)).toThrow('TOKEN_ENCRYPTION_SECRET must be set');
