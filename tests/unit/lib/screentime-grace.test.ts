...
describe('screentime-grace', () => {
  ...
  describe('countGraceUses', () => {
    it('should count grace logs for date range', async () => {
      ...
      dbMock.gracePeriodLog.count.mockResolvedValue(3);

      const count = await countGraceUses('member-1', startDate, endDate);

      expect(count).toBe(3);
      expect(dbMock.gracePeriodLog.count).toHaveBeenCalledWith({
        where: {
          memberId: 'member-1',
          requestedAt: {
            gte: startDate,
    ...
    it('should filter by status (PENDING, DEDUCTED, etc.)', async () => {
      ...
      dbMock.gracePeriodLog.count.mockResolvedValue(2);
  ...
  describe('processGraceRepayment', () => {
    it('should deduct from next week allocation (DEDUCT_NEXT_WEEK mode)', async () => {
      ...
      dbMock.gracePeriodLog.findMany.mockResolvedValue(pendingLogs);
      ...
      dbMock.gracePeriodLog.update.mockResolvedValue(pendingLogs[0]);
      ...
      // Verify logs updated
      expect(dbMock.gracePeriodLog.update).toHaveBeenCalledTimes(2);
    });
  ...
  describe('getOrCreateGraceSettings', () => {
    it('should return existing settings if found', async () => {
      ...
      dbMock.screenTimeGraceSettings.findUnique.mockResolvedValue(
        existingSettings
      );
      ...
      expect(dbMock.screenTimeGraceSettings.create).not.toHaveBeenCalled();
    });
    ...
    it('should create default settings if not found', async () => {
      ...
      dbMock.screenTimeGraceSettings.create.mockResolvedValue(
        defaultSettings
      );
      ...
      expect(dbMock.screenTimeGraceSettings.create).toHaveBeenCalledWith({
        data: {
          memberId: 'member-1',
          gracePeriodMinutes: 15,
          maxGracePerDay: 1,
