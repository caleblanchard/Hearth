...
describe('lib/achievements.ts', () => {
  ...
  describe('checkAndAwardAchievement', () => {
    ...
    it('should return null if achievement not found', async () => {
      dbMock.achievement.findUnique.mockResolvedValue(null)
    ...
    it('should create user achievement if not exists', async () => {
      dbMock.achievement.findUnique.mockResolvedValue(mockAchievement as any)
      dbMock.userAchievement.findUnique.mockResolvedValue(null)
      dbMock.userAchievement.create.mockResolvedValue({
        id: 'ua-1',
        userId,
        achievementId: 'ach-1',
        progress: 1,
      ...
      expect(dbMock.userAchievement.create).toHaveBeenCalledWith({
        data: {
          userId,
          achievementId: 'ach-1',
          progress: 1,
    ...
    it('should update user achievement if progress increased', async () => {
      dbMock.achievement.findUnique.mockResolvedValue(mockAchievement as any)
      dbMock.userAchievement.findUnique.mockResolvedValue({
        id: 'ua-1',
        userId,
        achievementId: 'ach-1',
        progress: 0,
      ...
      dbMock.userAchievement.update.mockResolvedValue({
        id: 'ua-1',
        progress: 1,
        isCompleted: true,
        completedAt: new Date(),
      ...
      expect(dbMock.userAchievement.update).toHaveBeenCalledWith({
        where: { id: 'ua-1' },
        data: {
          progress: 1,
          isCompleted: true,
  ...
  describe('updateStreak', () => {
    ...
    it('should create new streak if not exists', async () => {
      ...
      dbMock.streak.create.mockResolvedValue({
        id: 'streak-1',
        userId,
        type: streakType,
        currentCount: 1,
      ...
      expect(result).toBeDefined()
      expect(dbMock.streak.create).toHaveBeenCalledWith({
        data: {
          userId,
          type: streakType,
          currentCount: 1,
    ...
    it('should increment streak on consecutive day', async () => {
      ...
      dbMock.streak.findUnique.mockResolvedValue({
        id: 'streak-1',
        userId,
        type: streakType,
        currentCount: 5,
      ...
      dbMock.streak.update.mockResolvedValue({
        id: 'streak-1',
        currentCount: 6,
        longestCount: 6,
        lastActivityDate: today,
      ...
      expect(result).toBeDefined()
      expect(dbMock.streak.update).toHaveBeenCalledWith({
        where: { id: 'streak-1' },
        data: {
          currentCount: 6,
          longestCount: 6,
    ...
    it('should not update if same day', async () => {
      ...
      expect(result).toBeDefined()
      expect(dbMock.streak.update).not.toHaveBeenCalled()
    })
  ...
  describe('initializeAchievements', () => {
    it('should upsert all achievement definitions', async () => {
      dbMock.achievement.upsert.mockResolvedValue({} as any)

      await initializeAchievements()

      expect(dbMock.achievement.upsert).toHaveBeenCalledTimes(
    ...
    it('should create achievement if not exists', async () => {
      ...
      expect(dbMock.achievement.upsert).toHaveBeenCalledWith({
        where: { key: def.key },
        update: {
          name: def.name,
