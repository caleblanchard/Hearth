...
describe('/api/cron/generate-chore-instances', () => {
  ...
  describe('GET', () => {
    it('should allow request if CRON_SECRET is not set', async () => {
      ...
      dbMock.choreSchedule.findMany.mockResolvedValue([])
    ...
    it('should generate instances for FIXED assignment type', async () => {
      ...
      dbMock.choreSchedule.findMany.mockResolvedValue([mockSchedule] as any)
      getNextDueDates.mockReturnValue(dueDates)
      dbMock.choreInstance.findFirst
    ...
    it('should skip existing instances', async () => {
      ...
      dbMock.choreSchedule.findMany.mockResolvedValue([mockSchedule] as any)
      getNextDueDates.mockReturnValue(dueDates)
      dbMock.choreInstance.findFirst.mockResolvedValue({ id: 'existing-instance' } as any)
    ...
    it('should return 500 on critical error', async () => {
      ...
      dbMock.choreSchedule.findMany.mockRejectedValue(new Error('Database error'))
