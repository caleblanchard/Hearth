...
describe('/api/screentime/stats', () => {
  ...
  describe('GET', () => {
    ...
    it('should return stats for child (own data)', async () => {
      ...
      dbMock.screenTimeTransaction.findMany.mockResolvedValue(mockTransactions as any)
      dbMock.screenTimeBalance.findUnique.mockResolvedValue(mockBalance as any)
      dbMock.screenTimeSettings.findUnique.mockResolvedValue(mockSettings as any)
      ...
      expect(dbMock.screenTimeTransaction.findMany).toHaveBeenCalledWith({
        where: {
          memberId: 'child-1',
          type: 'SPENT',
          createdAt: {
    ...
    it('should filter by month period', async () => {
      ...
      const call = dbMock.screenTimeTransaction.findMany.mock.calls[0]?.[0]
    ...
    it('should return 500 on error', async () => {
      ...
      dbMock.screenTimeTransaction.findMany.mockRejectedValue(new Error('Database error'))
