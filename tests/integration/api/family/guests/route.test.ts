...
describe('/api/family/guests', () => {
  ...
  describe('GET /api/family/guests', () => {
    ...
    it('should return all active guest invites', async () => {
      ...
      dbMock.guestInvite.findMany.mockResolvedValue(mockInvites as any);
      ...
      expect(dbMock.guestInvite.findMany).toHaveBeenCalledWith({
        where: {
          familyId: 'family-test-123',
          status: {
            in: ['PENDING', 'ACTIVE'],
    ...
    it('should return empty array when no invites exist', async () => {
      dbMock.guestInvite.findMany.mockResolvedValue([]);
    ...
    it('should handle database errors gracefully', async () => {
      dbMock.guestInvite.findMany.mockRejectedValue(new Error('Database error'));
