...
describe('/api/family/sick-mode/start', () => {
  ...
  describe('POST', () => {
    ...
    it('should allow parents to start sick mode for any family member', async () => {
      ...
      dbMock.sickModeSettings.findUnique.mockResolvedValue(mockSickModeSettings as any);
      ...
      expect(dbMock.sickModeInstance.create).toHaveBeenCalledWith({
        data: {
          familyId: 'family-test-123',
          memberId: 'child-test-123',
          triggeredBy: 'MANUAL',
    ...
    it('should return 409 if sick mode already active for member', async () => {
      ...
      dbMock.sickModeInstance.findFirst.mockResolvedValue(mockSickModeInstance as any);
