...
describe('/api/medications/dose', () => {
  ...
  describe('POST', () => {
    ...
    it('should return 404 if medication safety config not found', async () => {
      dbMock.medicationSafety.findUnique.mockResolvedValue(null);
    ...
    it('should return 403 if member belongs to different family', async () => {
      dbMock.medicationSafety.findUnique.mockResolvedValue({
    ...
    it('should return 400 if daily maximum reached', async () => {
      dbMock.medicationSafety.findUnique.mockResolvedValue(mockMedicationSafety as any);

      // Mock 5 doses today (equal to maxDosesPerDay)
      dbMock.medicationDose.count.mockResolvedValue(5);
    ...
    it('should log dose successfully when safe', async () => {
      dbMock.medicationSafety.findUnique.mockResolvedValue(mockMedicationSafety as any);
      dbMock.medicationDose.count.mockResolvedValue(0);
      ...
      dbMock.medicationDose.create.mockResolvedValue(newDose as any);
      ...
      expect(dbMock.medicationDose.create).toHaveBeenCalledWith({
      ...
      expect(dbMock.medicationSafety.update).toHaveBeenCalledWith({
        where: { id: 'med-1' },
        data: expect.objectContaining({
    ...
    it('should allow override with valid reason by parent', async () => {
      ...
      dbMock.medicationDose.create.mockResolvedValue(overrideDose as any);
      dbMock.medicationSafety.update.mockResolvedValue({} as any);
      dbMock.auditLog.create.mockResolvedValue({} as any);
    ...
    it('should calculate next dose time correctly', async () => {
      ...
      dbMock.medicationSafety.findUnique.mockResolvedValue(medWith6HourInterval as any);
      dbMock.medicationDose.count.mockResolvedValue(0);
      dbMock.medicationDose.create.mockResolvedValue({
        id: 'dose-3',
        medicationSafetyId: 'med-1',
        givenAt: new Date(),
        givenBy: 'parent-test-123',
      ...
      const updateCall = (dbMock.medicationSafety.update as jest.Mock).mock.calls[0][0];
    ...
    it('should handle database errors gracefully', async () => {
      dbMock.medicationSafety.findUnique.mockRejectedValue(new Error('Database error'));
