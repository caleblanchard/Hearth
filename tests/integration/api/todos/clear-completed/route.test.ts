...
describe('/api/todos/clear-completed', () => {
  ...
  describe('DELETE', () => {
    ...
    it('should clear completed todos for family', async () => {
      ...
      dbMock.todoItem.deleteMany.mockResolvedValue({ count: 5 })
      ...
      expect(dbMock.todoItem.deleteMany).toHaveBeenCalledWith({
        where: {
    ...
    it('should handle singular message correctly', async () => {
      ...
      dbMock.todoItem.deleteMany.mockResolvedValue({ count: 1 })
    ...
    it('should handle zero completed todos', async () => {
      ...
      dbMock.todoItem.deleteMany.mockResolvedValue({ count: 0 })
    ...
    it('should return 500 on error', async () => {
      ...
      dbMock.todoItem.deleteMany.mockRejectedValue(new Error('Database error'))
