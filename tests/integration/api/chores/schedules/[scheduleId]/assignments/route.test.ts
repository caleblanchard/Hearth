...
import { AssignmentType } from '@/lib/enums'
...
describe('/api/chores/schedules/[scheduleId]/assignments', () => {
  ...
  describe('POST', () => {
    ...
    it('should return 400 if memberId is missing', async () => {
      ...
      dbMock.choreSchedule.findUnique.mockResolvedValue(getMockSchedule(session) as any)
    ...
    it('should return 400 if member is already assigned', async () => {
      ...
      dbMock.choreSchedule.findUnique.mockResolvedValue(mockScheduleWithAssignment as any)
    ...
    it('should create assignment successfully', async () => {
      ...
      dbMock.choreAssignment.create.mockResolvedValue(mockNewAssignment as any)
    ...
    it('should auto-assign rotationOrder for ROTATING type', async () => {
      ...
      dbMock.choreSchedule.findUnique.mockResolvedValue(mockScheduleWithAssignments as any)
      dbMock.familyMember.findUnique.mockResolvedValue({
        id: 'child-2',
        familyId: session.user.familyId,
      } as any)
      dbMock.choreAssignment.create.mockResolvedValue({
        ...mockNewAssignment,
        rotationOrder: 3,
      } as any)
      ...
      expect(response.status).toBe(200)
      expect(dbMock.choreAssignment.create).toHaveBeenCalledWith(
    ...
    it('should return 500 on error', async () => {
      ...
      dbMock.choreAssignment.create.mockRejectedValue(new Error('Database error'))
