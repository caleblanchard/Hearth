...
describe('POST /api/cron/sick-mode-auto-disable', () => {
  ...
  it('should disable sick mode instances that have exceeded auto-disable duration', async () => {
    ...
    dbMock.sickModeSettings.findMany.mockResolvedValue([
      {
        id: 'settings-1',
        familyId: 'family-1',
    ...
    dbMock.sickModeInstance.findMany.mockResolvedValue([expiredInstance] as any);
  ...
  it('should NOT disable instances within the auto-disable window', async () => {
    ...
    dbMock.sickModeInstance.findMany.mockResolvedValue([] as any);
    ...
    expect(dbMock.sickModeInstance.update).not.toHaveBeenCalled();
  });
  ...
  it('should handle multiple families with different auto-disable settings', async () => {
    ...
    expect(dbMock.sickModeInstance.update).toHaveBeenCalledTimes(1);
    expect(dbMock.sickModeInstance.update).toHaveBeenCalledWith({
      where: { id: 'sick-1' },
      data: {
        isActive: false,
  ...
  it('should return success when no families have auto-disable configured', async () => {
    ...
    dbMock.sickModeSettings.findMany.mockResolvedValue([] as any);
    ...
    expect(data.disabledCount).toBe(0);
    expect(data.disabledInstances).toHaveLength(0);
    expect(dbMock.sickModeInstance.findMany).not.toHaveBeenCalled();
  });
});
