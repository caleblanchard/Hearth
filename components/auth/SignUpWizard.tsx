...
export function SignUpWizard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!authData.session) {
        // Redirect to signin with a message about email confirmation
        router.push(`/auth/signin?confirmEmail=true&email=${encodeURIComponent(email.trim().toLowerCase())}`);
        return;
      }

      // Success! User is already confirmed (shouldn't happen in production with email confirmation enabled)
    ...
    <div className="w-full max-w-md space-y-8">
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        <button
          ...
          disabled={loading}
          className="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-3 text-gray-900 dark:text-gray-100 font-medium hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 disabled:opacity-50 transition-colors"
        >
